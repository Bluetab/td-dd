  ...
  defp validate_params(changeset) do
    ...
    if params == %{} or params == nil do
      ...
      with true <- length(Map.keys(params)) == length(@available_params_keys),
           true <- length(Map.keys(get_change(changeset, :params))) == length(@available_params_keys) do
        changeset
      else
        false ->
          changeset
          |> add_error(:params, "Invalid params. Available params: #{inspect(@available_params_keys)}")
      end
    end
  end
