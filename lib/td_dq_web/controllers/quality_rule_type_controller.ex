defmodule TdDqWeb.QualityRuleTypeController do
  ...
  swagger_path :create do
    ...
    parameters do
      quality_rule_type(
        :body,
        Schema.ref(:QualityRuleTypeCreate),
  ...
  def create(conn, %{"quality_rule_type" => quality_rule_type_params}) do
    with {:ok, %QualityRuleType{} = quality_rule_type} <-
           QualityRules.create_quality_rule_type(quality_rule_type_params) do
      conn
      |> put_status(:created)
  ...
  swagger_path :update do
    ...
    parameters do
      quality_rule_type(
        :body,
        Schema.ref(:QualityRuleTypeUpdate),
  ...
  def update(conn, %{"id" => id, "quality_rule_type" => quality_rule_type_params}) do
    ...
    with {:ok, %QualityRuleType{} = quality_rule_type} <-
           QualityRules.update_quality_rule_type(quality_rule_type, quality_rule_type_params) do
      render(conn, "show.json", quality_rule_type: quality_rule_type)
    end
  end
