defmodule TdDqWeb.RuleTypeControllerTest do
  ...
  describe "index" do
    ...
    test "lists all rule_type", %{conn: conn} do
      conn = get(conn, Routes.rule_type_path(conn, :index))
  ...
  describe "create rule_type" do
    ...
    test "renders rule_type when data is valid", %{conn: conn} do
      conn = post conn, Routes.rule_type_path(conn, :create), rule_type: @create_attrs
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = recycle_and_put_headers(conn)

      conn = get(conn, Routes.rule_type_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, Routes.rule_type_path(conn, :create), rule_type: @invalid_attrs
  ...
  describe "update rule_type" do
    ...
    test "renders rule_type when data is valid", %{
      ...
      conn = put conn, Routes.rule_type_path(conn, :update, rule_type), rule_type: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, rule_type: rule_type} do
      conn = put conn, Routes.rule_type_path(conn, :update, rule_type), rule_type: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete rule_type" do
    ...
    test "deletes chosen rule_type", %{conn: conn, rule_type: rule_type} do
      conn = delete(conn, Routes.rule_type_path(conn, :delete, rule_type))
      assert response(conn, 204)

      conn = recycle_and_put_headers(conn)

      assert_error_sent 404, fn ->
        get(conn, Routes.rule_type_path(conn, :show, rule_type))
      end
    end
  end
