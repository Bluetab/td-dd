defmodule TdDdWeb.DataStructures.XLSXControllerTest do
  ...
  describe "download" do
    ...
    @tag authentication: [role: "admin"]
    test "downloads xlsx content using scroll to search", %{conn: conn} do
      ...
      assert %{resp_body: body} =
               post(conn, Routes.xlsx_path(conn, :download, %{download_type: "editable"}))
  ...
  describe "upload" do
    @tag authentication: [role: "admin"]
    test "creates async job for upload processing", %{conn: conn, claims: claims} do
      ...
      assert conn
             |> post(Routes.xlsx_path(conn, :upload), structures: upload(file))
             |> response(:accepted)
