defmodule TdQd.RuleTypeTest do
  ...
  defwhen ~r/^"(?<user_name>[^"]+)" tries to create a Rule named "(?<qrt_name>[^"]+)" with following data:$/,
          %{user_name: user_name, qrt_name: qrt_name, table: table},
          state do
    token = state[:token]
  ...
  defand ~r/^"(?<user_name>[^"]+)" is able to view rule type named "(?<qrt_name>[^"]+)" with following data:$/,
         %{user_name: user_name, qrt_name: qrt_name},
         state do
    assert user_name == state[:user_name]
